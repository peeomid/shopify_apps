...
class AppStoreSpider(LastmodSpider):
    ...
    APP_REGEX = r"https://apps\.shopify\.com/([^/]+)$"
    CATEGORY_REGEX = r"https://apps\.shopify\.com/categories/(.*?)"
    DEVELOPER_REGEX = r"https://apps\.shopify\.com/partners/(.*?)"
    APP_GROUP_REGEX = r"https://apps\.shopify\.com/app-groups/extensions/(.*?)"
    STORIES = r"https://apps\.shopify\.com/stories/(.*?)"

    allowed_domains = ['apps.shopify.com']
    sitemap_urls = ['https://apps.shopify.com/sitemap.xml']
    sitemap_rules = [
        (re.compile(APP_REGEX), 'parse_app'),
        (re.compile(CATEGORY_REGEX), 'parse_category'),
        (re.compile(DEVELOPER_REGEX), 'parse_developer'),
        (re.compile(APP_GROUP_REGEX), 'parse_app_group'),
        (re.compile(STORIES), 'parse_stories'),
    ]
